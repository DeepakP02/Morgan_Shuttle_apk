...
export const GlobalProvider = ({ children }) => {
  ...
  const mergeDriverSession = useCallback(async (partial) => {
    ...
    setDriverSession((prev) => {
      ...
      if (user?.id) {
        ...
        ).catch(err => console.error('Failed to save driver session:', err));
      }
      return next;
    });
  }, [user?.id]);
  ...
  const startDriverShift = async () => {
    ...
    await mergeDriverSession({ shiftActive: true, paused: false });
  };
  ...
  const endDriverShift = async () => {
    ...
    await mergeDriverSession({ shiftActive: false, paused: false, trackingEnabled: true });
  };
  ...
  const setDriverTrackingEnabled = async (enabled) => {
    ...
    await mergeDriverSession({ trackingEnabled: !!enabled });
  };
  ...
  const setDriverTrackingPaused = async (paused) => {
    ...
    await mergeDriverSession({ paused: !!paused });
  };
  ...
  const logout = useCallback(() => {
    ...
    Location.hasStartedLocationUpdatesAsync(LOCATION_TASK_NAME)
      ...
      .catch(err => console.warn('Logout location cleanup failed:', err));
  ...
  const postShuttleDriverLocation = async (lat, lng) => {
    ...
    try {
      ...
      // Quiet fail to avoid spamming logs during poor connection
      console.log('Shuttle location ping failed (silent):', error?.message);
    }

    // Update the live map representation
    if (liveFromServer) {
      setShuttleLive((prev) => ({
        ...(prev || {}),
        ...(liveFromServer || {}),
        lat: liveFromServer.lat ?? lat,
        lng: liveFromServer.lng ?? lng,
        status: liveFromServer.status || 'active',
        updatedAt: liveFromServer.updated_at || new Date().toISOString(),
      }));
    } else {
      // Fallback update if server response is missing
      setShuttleLive((prev) => ({
        ...(prev || {}),
        lat,
        lng,
    ...
    // If there's an in-progress trip, update it too
    const list = tripsRef.current || [];
    const running = list.find((t) => t && t.status === 'in_progress');
    if (running) {
      try {
        await updateTripLocation(running.id, lat, lng);
      } catch (_) {}
    }
  };
  // ─────────────────────────────────────────────────────────────────────────────
