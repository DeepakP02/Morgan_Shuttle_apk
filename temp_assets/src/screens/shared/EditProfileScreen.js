      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.avatarSection}>
           <View style={styles.avatar}>
             <Text style={styles.avatarText}>{formData.name ? formData.name.charAt(0).toUpperCase() : '?'}</Text>
           </View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 92, height: 92, borderRadius: 32, backgroundColor: COLORS.white, justifyContent: 'center', alignItems: 'center', marginBottom: 15, ...SHADOWS.medium },
  avatarText: { fontSize: 36, fontWeight: '900', color: COLORS.primary },
  changeLabel: { color: COLORS.secondary, fontWeight: '900', fontSize: 12, textTransform: 'uppercase', letterSpacing: 2 },
  form: { gap: 25, marginBottom: 40 },
  inputGroup: { gap: 12 },
  label: { fontSize: 11, fontWeight: '900', color: COLORS.gray[300], letterSpacing: 2, textTransform: 'uppercase', marginLeft: 5 },
  input: { backgroundColor: COLORS.white, borderRadius: 20, padding: 20, fontSize: 16, color: COLORS.black, fontWeight: '700', borderWidth: 1, borderColor: COLORS.gray[50], ...SHADOWS.soft },
  saveBtn: { 
    height: 65, 
    borderRadius: 24, 
