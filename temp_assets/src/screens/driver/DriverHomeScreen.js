...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, StatusBar, Alert, Modal, TextInput, Platform, Image, KeyboardAvoidingView, ScrollView, Switch } from 'react-native';
...
export const DriverHomeScreen = () => {
  ...
  // Shift-based live GPS (independent of booking / trip id)
  useEffect(() => {
    const shouldPing =
    ...
    const tick = async () => {
      try {
        ...
        await postShuttleDriverLocation(loc.coords.latitude, loc.coords.longitude);
      } catch (err) {
        console.warn('Shuttle GPS tick:', err?.message);
      }
    };

    tick();
    pingRef.current = setInterval(tick, 8000);
    return clearPing;
  }, [
  ...
  const ensureLocationPermission = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert(t('common.error'), t('driver.location_required') || 'Location permission is required for shuttle tracking.');
      return false;
    }
    return true;
  };
